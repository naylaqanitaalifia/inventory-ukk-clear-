...
<div
    {{-- x-dsatsa="{ open: false, id: '', name: ''}" --}}
    @modal-open.window="open = true, id = $event.detail.id, name = $event.detail.name"
    class="">
    <x-ui.modal>
        <div
            ...
            <form :action="`{{ url($route) }}/${id}`" method="POST" class="flex flex-col gap-3 sm:flex-row-reverse sm:justify-center">
                @csrf
                @method('DELETE')
